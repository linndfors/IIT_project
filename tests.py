...
class TestSteganography(unittest.TestCase):
    ...
    def test_encode_decode_success(self):
        ...
        result = lsb_stego.encode_lsb(
            self.TEST_FILE, self.PROTECTED_FILE, secret_payload
        )
        ...
        self.assertEqual(
            decoded_msg,
            secret_payload,
            "Декодоване повідомлення має співпадати з оригіналом",
        )
...
class TestDatabaseModels(unittest.TestCase):
    ...
    def test_track_relationship(self):
        ...
        track = AudioTrack(
            title="Hit Song", artist="Me", owner=user, filename="song.wav"
        )
    ...
    def test_watermark_record(self):
        ...
        wm = WatermarkRecord(
            track=track, watermark_payload="UUID-CODE-123", pdf_certificate="cert.pdf"
        )
        db.session.add(wm)
        db.session.commit()

        fetched_wm = WatermarkRecord.query.filter_by(
            watermark_payload="UUID-CODE-123"
        ).first()
    ...
    def test_track_watermark_relationship(self):
        ...
        db.session.commit()
        wm = WatermarkRecord(
            track=track, watermark_payload="WM-1", pdf_certificate="c.pdf"
        )
        db.session.add(wm)
        db.session.commit()
